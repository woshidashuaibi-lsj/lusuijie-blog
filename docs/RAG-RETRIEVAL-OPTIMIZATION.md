...
**逐步拆解计算过程（以混合检索两路为例，k=60）：**

以上面检索示例中的三个文档为例：

| 文档 | 路线A排名 | 路线B排名 | RRF计算 | 最终得分 |
|------|---------|---------|---------|---------|
| 文档1 | 第1名 | 第2名 | 1/(60+1) + 1/(60+2) = 1/61 + 1/62 | **0.03252** |
| 文档2 | 未命中 | 第1名 | 0 + 1/(60+1) = 1/61 | 0.01639 |
| 文档5 | 第2名 | 未命中 | 1/(60+2) + 0 = 1/62 | 0.01613 |

最终排序：文档1 > 文档2 > 文档5

**为什么文档2 排在文档5 前面？**

两者都只在一条路线命中，但文档2 在路线B是**第1名**（`1/61=0.01639`），文档5 在路线A是**第2名**（`1/62=0.01613`），第1名比第2名得分高，所以文档2略胜。

**k=60 的作用——防止头部垄断：**

```
不加 k（k=0）：            加 k=60 后：
第1名：1/1  = 1.000        第1名：1/61 = 0.01639
第2名：1/2  = 0.500        第2名：1/62 = 0.01613
第3名：1/3  = 0.333        第3名：1/63 = 0.01587
```

不加 k 时，第1名得分是第2名的 2 倍，排名靠前的文档会绝对统治结果。
加上 k=60 后，各排名之间的得分差距被大幅压缩，让"两路都靠前但不是第1"的文档也能排到高位。

**为什么不直接把两条路线的分数相加？**

向量相似度（如 0.87）和 BM25 分数（如 23.4）**量纲完全不同**，直接相加没有意义。RRF 只看排名不看分数，排名是统一的（都是第1、第2、第3...），所以可以跨路线比较和融合。
