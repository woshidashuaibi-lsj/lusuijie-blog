...
export default function BookReader({ book }: BookReaderProps) {
  // 初始值固定为 0，避免 SSR 与客户端不一致导致 hydration 错误
  const [currentChapterIdx, setCurrentChapterIdx] = useState(0);
  const [showSettings, setShowSettings] = useState(false);
  // settings 也用默认值初始化，在 useEffect 中读取本地存储，避免 hydration 错误
    ...
    <div className={styles.reader} style={{ background: theme.bg, color: theme.color }}>
      ...
      <div className={styles.topBar} style={{ borderBottomColor: theme.chapterBg }}>
        ...
        <div className={styles.topBarActions}>
          {AI_CHAT_ENABLED_SLUGS.includes(book.slug) && (
            <Link
              href={`/book/${book.slug}/chat`}
              className={styles.chatBtn}
              style={{ color: settings.background === 'light' ? '#6366f1' : theme.color }}
              title="与书对话"
            >
              ✦ 与书对话
            </Link>
